...
public class SvnClient {
    ...
    public void checkout(ISVNEventHandler handler) throws SVNException {
        checkout(SVNRevision.HEAD,handler);
    }
     ...
     */
    public void checkout(SVNRevision revision,ISVNEventHandler handler) throws SVNException {
        try {
            lock.lock();
            // 相关变量赋值
            SVNURL repositoryURL = SVNURL.parseURIEncoded(svnUrl);
            ...
            updateClient.doCheckout(repositoryURL, wcDir, revision, revision, SVNDepth.INFINITY, false);
        } finally {
    ...
    public List<SvnNode> doCompareRevision(String filePath,long fromVersion,long toVersion) throws SVNException {
        if (!lock.tryLock()) {
            throw new LightException("正在执行其它任务!");
        }
        try {
    ...
    public SVNCommitInfo doRollback(long reversion) throws SVNException {
        ...
        try {
            if(!isWorkingCopy()) {
                throw new LightException(localPath+" is not workingCopy!");
            }
